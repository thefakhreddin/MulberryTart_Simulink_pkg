classdef Distance_Sensor < realtime.internal.SourceSampleTime ...
    ...
    methods
        % Constructor
        function obj = Distance_Sensor(varargin)
    ...
    methods (Access=protected)
        function setupImpl(~)
            if coder.target('Rtw')
                coder.cinclude('distanceSensorBlock.h');
                coder.ceval('distanceSensorInit');
            end
        end
        ...
        function y = stepImpl(obj)
            y = 0;
            if coder.target('Rtw')
                y = coder.ceval('distanceSensorRead', obj.sensorPort);
            end
        end
